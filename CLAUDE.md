...
- **Dependabot PRs cannot satisfy the required checks.** CodeQL **default setup** (no workflow file in the repo) does not run on Dependabot PRs — the aggregate check reports `conclusion=neutral, title="3 configurations not found"` and the three `Analyze (...)` contexts never appear, so 3 of 6 required checks are permanently missing. Known upstream limitation (`github/codeql-action#2858`). Consequence: an enabled auto-merge queues forever and manual merging needs a bypass, so **consolidate Dependabot bumps into a human-authored PR** (pip pins live in `requirements.lock.txt`) rather than merging them one by one. Dependabot then closes its own PRs once it sees the versions on `main` ("Looks like X is up-to-date now"), which makes a later `gh pr close` a silent no-op — comment first if you want the reason recorded on them.
